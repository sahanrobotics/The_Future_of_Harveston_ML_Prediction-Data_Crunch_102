...
TEST_FILE = 'test.csv'
SUBMISSION_FILE = 'submission_prophet_optuna_feats_v2.csv' # New filename
...
TARGET_VARIABLES_TO_PROCESS = BASE_TARGET_VARIABLES + ['Wind_Direction'] # Original list for preprocessing
TARGETS_FOR_MODELING = BASE_TARGET_VARIABLES + WIND_DIR_TARGETS # List Prophet will predict
...
MIN_VALIDATION_POINTS = 30 # Need enough points AFTER feature engineering NaNs removed
OUTLIER_CLIP_PERCENTILE = 0.01

# Optuna Configuration
N_OPTUNA_TRIALS = 30 # START SMALL (e.g., 15-25)! Increase later.
OPTUNA_TIMEOUT = None
...
# --- Helper Functions ---
def kelvin_to_celsius(temp_k): return temp_k - 273.15
...
def preprocess_data(df, is_train=True):
    ...
    if is_train:
        # --- Define columns needed initially ---
        ...
        # --- Feature Engineering: Wind Direction Sin/Cos ---
        if 'Wind_Direction' in df_processed.columns:
            ...
            # Add new targets to the list of columns we need
            all_current_cols.extend(WIND_DIR_TARGETS)
            all_current_cols = list(set(all_current_cols))

        # --- NaN/Inf Cleaning (BEFORE feature eng requires clean data) ---
        ...
        # --- Feature Engineering: Lags & Rolling Means (Grouped by Kingdom) ---
        df_processed.sort_values(['kingdom', 'ds'], inplace=True)
        print("Creating lag/rolling features (grouped by kingdom)...")
        lag_feat_names = []
        roll_feat_names = []
        grouped = df_processed.groupby('kingdom', group_keys=False) # Group once

        for target_col in TARGETS_FOR_MODELING: # Use modeling targets
            if target_col in df_processed.columns:
                ...
                # Rolling Means (using shift(1) inside to avoid data leakage)
        ...
        # Keep ds, kingdom, modeling targets, base regressors, geo regressors, and NEW features
        cols_to_keep = (['ds', 'kingdom']
                       + TARGETS_FOR_MODELING
                       + NON_GEO_REGRESSORS
                       + GEO_REGRESSORS
                       + all_feature_cols)
        # Ensure we only try to keep columns that actually exist after all processing
        final_cols_to_keep = [col for col in cols_to_keep if col in df_processed.columns]
        # Ensure original Wind_Direction is dropped if sin/cos exist
...
def objective(trial, train_data, val_data, target_col,
    ...
    # Regressor Prior Scales (tuned per group)
    ...
    # Add regressors with group-specific prior scales
...
# 2. Preprocess Data (includes feature engineering)
...
# 4. Train, Validate (Optimize), and Predict
...
for kingdom in unique_kingdoms:
    ...
    # Get full training data for the kingdom, ensuring all engineered features are present
    ...
    # Check if enough data *after potential feature eng drops*
    ...
    # --- Get list of all potential feature columns once per kingdom ---
    ...
    for target in TARGETS_FOR_MODELING:
        ...
        # --- Identify Regressors for THIS target's validation ---
        # Exclude features derived DIRECTLY from the current target
        ...
        # --- Final Model Training (NO external/lag/roll regressors) ---
        ...
        # Add seasonalities using tuned orders or defaults
...
# --- Post-Processing: Convert Wind Direction back ---
...
if 'Wind_Direction_sin' in submission_df.columns and 'Wind_Direction_cos' in submission_df.columns:
    ...
    # Only calculate where both components are valid numbers
    valid_mask = sin_comp.notna() & cos_comp.notna()
    radians = np.full(len(submission_df), np.nan) # Initialize with NaN
...
for target in FINAL_SUBMISSION_COLUMNS[1:]: # Skip ID
    ...
    # Wind direction already handled
